...
def load_and_clean_kpindex_3hr(start_time="1932-01-01", end_time=datetime.today().strftime("%Y-%m-%d"), index="Kp"):
    """
    Downloads and cleans the Kp index data for the given date range.
    ...
    kpindex_df = pd.DataFrame(kpindex_df)

    kpindex_df = kpindex_df.rename(
        columns={"Date": "Datetime", "Daily_Kp_max": "Kp_Index"}
    )
